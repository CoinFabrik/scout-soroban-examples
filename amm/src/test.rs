...
mod cpamm {
    soroban_sdk::contractimport!(file = "cpamm/target/wasm32-unknown-unknown/release/cpamm.wasm");
}

mod csamm {
    soroban_sdk::contractimport!(file = "csamm/target/wasm32-unknown-unknown/release/csamm.wasm");
}
...
fn _create_csamm_contract<'a>(e: &Env) -> util::SwapCurveClient<'a> {
...
fn _create_token_contract<'a>(e: &Env) -> TokenClient<'a> {
    TokenClient::new(
        e,
        &e.register_contract_wasm(None, soroban_token_contract::WASM),
    )
}
...
fn create_token_pair<'a>(e: &Env, admin: &Address) -> (TokenAdminClient<'a>, TokenClient<'a>) {
    ...
    (
        TokenAdminClient::new(&e, &contract),
...
fn test_no_init_1() {
    ...
    let token_admin_client1 =
        TokenAdminClient::new(&env, &env.register_stellar_asset_contract(token_admin1));
    let token_admin_client2 =
        TokenAdminClient::new(&env, &env.register_stellar_asset_contract(token_admin2));

    let admin = Address::generate(&env);

    contract.new_swap(
        &admin,
        &token_admin_client1.address,
        &token_admin_client2.address,
        &cpamm.address,
    );
}
...
fn test_bad_swap() {
    ...
    let (token_admin_client1, token1) = create_token_pair(&env, &token_admin1);
    let (token_admin_client2, token2) = create_token_pair(&env, &token_admin2);
    let (_token_admin_client3, token3) = create_token_pair(&env, &token_admin3);
    ...
    let _received = contract.swap(&alice, &alice, &token1.address, &token3.address, &1_000, &0);
}
