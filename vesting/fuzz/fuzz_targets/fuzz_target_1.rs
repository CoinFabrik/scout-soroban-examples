...
extern crate vesting_contract;

use vesting_contract::*;
...
fuzz_target!(|input: Input| {
    ...
    let total_vested = vesting.contract.add_vest(&id, &vesting.token.address, &vesting.admin, &input.vest_amount);
            ...
            (
                ...
                AuthorizedInvocation {
                    function: AuthorizedFunction::Contract((
                        ...
                        (id.clone(), vesting.token.address.clone(), vesting.admin.clone(), input.vest_amount).into_val(env),
                    )),
                    ...
                    sub_invocations: std::vec![
                        AuthorizedInvocation {
                            function: AuthorizedFunction::Contract((
                                ...
                                (vesting.admin.clone(), vesting.contract.address.clone(), input.vest_amount).into_val(env),
                            )),
                            sub_invocations: std::vec![]
                        }
                    ]
    ...
    assert_eq!(total_vested, input.vest_amount);
    assert_eq!(vesting.contract.retrievable_balance(&id), 0);
    ...
    assert_eq!(vesting.contract.retrievable_balance(&id), input.vest_amount / 86400);

    vesting.env.ledger().with_mut(|info|{
        info.timestamp += 86400 / 2 - 1;
    });

    assert_eq!(vesting.contract.retrievable_balance(&id), input.vest_amount / 2);

    let paid_out = vesting.contract.pay_out(&id);

    assert_eq!(paid_out, input.vest_amount / 2);
    assert_eq!(vesting.contract.retrievable_balance(&id), 0);

    let total_vested = vesting.contract.add_vest(&id, &vesting.token.address, &vesting.admin, &1_500_000_000);

    assert_eq!(total_vested, 1_500_000_000 + input.vest_amount);
    assert_eq!(vesting.contract.retrievable_balance(&id), (1_500_000_000 + input.vest_amount) / 2 - paid_out);

    vesting.env.ledger().with_mut(|info|{
        info.timestamp += 86400;
    });

    assert_eq!(vesting.contract.retrievable_balance(&id), 1_500_000_000 + input.vest_amount);
});
